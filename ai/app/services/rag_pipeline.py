...
from app.services.evaluator import evaluate_retrieval, evaluate_answer, evaluate_faithfulness

def get_rag_answer(question: str) -> dict:
    """
    질문 → 검색 → 프롬프트 생성 → 답변 생성 → 평가
    """
    if DB_BACKEND == "chroma":
    ...
    # === 평가 메트릭 추가 ===
    retrieval_score = evaluate_retrieval(question, contexts)
    ...
    return {
        ...
        "metrics": {
            ...
            "qa_similarity": answer_score,
            "faithfulness_similarity": faithfulness_score
        }
    }
