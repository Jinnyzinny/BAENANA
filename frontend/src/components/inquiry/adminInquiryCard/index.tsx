...
export function AdminInquiryCard({
  ...
  const { data } = useGetAdminInquiryDetail(inquiryId);
  const { mutate: addAdminInquiry } = useAddAdminInquiry();
  const { mutate: editAdminInquiry } = useEditAdminInquiry();
  const { mutate: deleteAdminInquiry } = useDeleteAdminInquiry();

  // 저장
  function handleSave() {
    if (!content) return Alert.alert("입력 오류", "내용을 입력해주세요.");

    console.log("inquiryId: ", inquiryId);
    addAdminInquiry(
      { inquiryId, answerContent: content },
      {
        onSuccess: () => {
          setContent("");
          setIsOpen(false);
          ...
          {!isOpen && status === "PENDING" && (
            <View className="gap-5">
              ...
              <TouchableOpacity onPress={() => setIsOpen(true)}>
                <View className="p-3 bg-violet-400 rounded-lg items-center">
